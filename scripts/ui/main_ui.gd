...
func _ready() -> void:
	...
	_refresh_hud()
	_apply_ui_scale()
	_append_system_message("%s 的农场 · 第 %d 天 · %s %s" % [
...
func _on_name_prompt_confirmed(name: String) -> void:
	...
	if cleaned == "":
		return
	_append_companion_message("嗯，%s。我记住了。" % cleaned)
	_refresh_hud()
...
func _on_time_changed(time_of_day: String) -> void:
	...
	if time_of_day == GameState.TIME_NIGHT:
		_append_system_message("夜幕降临。可以睡觉进入下一天，或继续逛逛。")
	elif time_of_day == GameState.TIME_EVENING:
		_append_system_message("天色渐晚。")


func _refresh_hud() -> void:
	var week_label := "周目 %d · 第 %d/%d 天 · %s · %s" % [
		GameState.get_week_index(),
		GameState.get_loop_day(),
		7,
		GameState.get_weather_label(),
		GameState.get_time_label(),
	]
	...
	_hud_stats.text = "金币 %d | 萝卜种子 %d | 萝卜 %d | 零食 %d | 种子价 %d" % [
		GameState.coins,
		GameState.get_item_count("turnip_seed"),
		GameState.get_item_count("turnip"),
		_count_treats(),
		GameState.get_seed_buy_price(),
	]
	_hud_stage.text = "亲密度 %d | 默契 %d | %s · 互动 %.0f%% · %s" % [
		GameState.affection,
		GameState.bond,
		StoryBeatDirector.get_route_label(),
		RelationshipDirector.get_interaction_score() * 100.0,
...
func _maybe_show_awakening() -> void:
	...
	if _story_choice_blocked or _ending_panel.visible:
		return
	_append_system_message(StoryNodeCopy.get_awakening("open"))
...
func _maybe_force_story_finale() -> void:
	...
	_append_system_message("—— 五周故事收束（兜底结局）——")
...
func _on_story_beat_finished(beat_id: String) -> void:
	...
	if _companion_snuggle_pending_beat_id != "":
		...
		if beat_id.strip_edges() != "":
			...
			_append_system_message("—— 主线节点 %s ——" % beat_id)
		_story_beat_blocked = false
...
func _on_story_beat_choice(choice_id: String) -> void:
	...
	if beat_id == "P_N06p":
		...
		return
	if beat_id.ends_with("_N14n") or beat_id.ends_with("_N17"):
...
func _on_story_route_changed(old_route: String, new_route: String) -> void:
	...
	if msg.strip_edges() == "":
		return
	_append_system_message("—— 关系变化 ——")
	_append_system_message(msg)
	_append_system_message("当前路线：%s" % StoryBeatDirector.get_route_label(new_route))
...
func on_tree_hollow_clicked() -> void:
	...
	if StoryBeatDirector.has_pending_night_beat():
		if not StoryBeatDirector.can_trigger_night_beat_at_hollow():
			_append_system_message(StoryNodeCopy.get_system("tree_hollow_night_wait"))
			return
		...
		return
	_append_system_message(StoryNodeCopy.get_system("tree_hollow_day"))
...
func on_plot_clicked(plot_id: int, _world_pos: Vector2) -> void:
	...
	if stage == 0:
		if GameState.plant_turnip(plot_id):
			_append_system_message("种下了萝卜种子。")
			_refresh_hud()
		else:
			_append_system_message("没有萝卜种子了，去商店购买吧。")
		return

	if GameState.can_harvest(plot_id):
		if GameState.harvest_turnip(plot_id):
			_append_system_message("收获了 1 个萝卜！")
			_refresh_hud()
		else:
			_append_system_message("这块田暂时收不了，稍后再试试。")
		return

	if stage >= GameState.MATURE_STAGE:
		_append_system_message("萝卜还没完全成熟，再等等吧。")
		return

	if plot_id in GameState.watered_plots or bool(plot.get("watered", false)):
		_append_system_message("这块田今天已经浇过了。")
		return

	if GameState.weather_today == GameState.WEATHER_RAIN:
		_append_system_message("今天下雨，田会自己喝饱水。")
		return

	GameState.mark_plot_watered(plot_id)
	_append_system_message("给这块田浇好了水。")
	_refresh_hud()


func on_door_clicked() -> void:
	CompanionDirector.notify_player_active()
	_append_system_message("这扇门通向外面，现在专心打理萝卜田就好。")


func on_shop_clicked() -> void:
	if TaskSystem.is_busy():
		_append_system_message("小狸还在忙，请等一下。")
		return
...
func on_need_closer() -> void:
	_append_system_message("再走近一点才能交互。")
...
func _on_next_day_pressed() -> void:
	if TaskSystem.is_busy():
		_append_system_message("任务还没结束，不能进入下一天。")
		return
	if GameState.is_story_complete():
		_append_system_message("故事已经结束了。")
		return
	if GameState.game_day >= GameState.FINAL_GAME_DAY:
		if not GameState.has_seen_awakening():
			_append_system_message("今天好像有什么想说的，先听小狸讲完吧。")
			call_deferred("_maybe_show_awakening")
			return
	...
	if StoryBeatDirector.has_unseen_weekend_night_beat():
		if StoryBeatDirector.can_trigger_night_beat_at_hollow():
			_append_system_message(StoryNodeCopy.get_system("tree_hollow_night_ready"))
		else:
			_append_system_message(StoryNodeCopy.get_system("tree_hollow_night_wait"))
		return
...
func _advance_to_next_day() -> void:
	if not GameState.can_advance_day():
		return
	var slept_at_night := GameState.is_night()
	GameState.advance_day()
	if slept_at_night:
		_append_system_message("睡到了新的一天。")
	_append_system_message("—— 周目 %d · 第 %d 天 · %s %s ——" % [
		GameState.get_week_index(),
		GameState.get_loop_day(),
		GameState.get_weather_label(),
		GameState.get_time_label(),
	])
	if GameState.last_day_summary.strip_edges() != "":
		_append_system_message(GameState.last_day_summary)


func _on_day_advanced() -> void:
	if GameState.game_day >= 10:
		StoryBeatDirector.ensure_story_route_locked()
	if GameState.game_day in [22, 29]:
		StoryBeatDirector.refresh_story_route()
...
func _on_week_reset(week_index: int) -> void:
	if week_index == 2:
		_append_system_message("—— 新的一周开始了 ——")
		_append_system_message("小狸看你的眼神，好像有点不一样。")
...
func sleep_from_companion() -> void:
	if _is_gameplay_locked():
		return
	if TaskSystem.is_busy():
		_append_system_message("任务还没结束，不能睡觉。")
		return
...
func _send_chat_async(text: String) -> void:
	...
	if IntentBridge.should_fallback(local_intent):
		_set_npc_busy(true)
		_chat_input.placeholder_text = "小狸在理解你的意思…"
...
func _on_npc_reply_ready(
	...
	_set_npc_busy(false)
	_append_companion_message(text)
	...
	if event == "player_chat":
		...
		if not _chat_action_handled:
			...
			if not _chat_action_handled:
				_try_execute_companion_followthrough(text, chat_api_intent)
		else:
			NpcBridge.take_chat_intent(request_id)
		_pending_chat_text = ""
		return
...
func _handle_companion_feed_reply(request_id: int, text: String, used_fallback: bool) -> void:
	...
	var committed_item: Dictionary = commit.get("item", item)
	var item_name := str(committed_item.get("name", "零食"))
	GameState.register_feed_reply(reply)
	_refresh_hud()
	_append_system_message("你给小狸喂了 %s。" % item_name)
	_append_companion_message(reply)
	_show_api_source_hint(request_id, used_fallback)
	_feed_panel.set_status_message("")
	_feed_panel.rebuild()
...
func _apply_relationship_delta(
	...
	if aff > 0:
		...
		_append_system_message("（%s）" % reason)
	if used_fallback and event == "player_chat":
		_append_system_message("（本地判定 · 未接大模型）")
...
func _show_api_source_hint(request_id: int, used_fallback: bool) -> void:
	...
	match source:
		"mock_fallback":
			...
			if reason != "":
				_append_system_message("（LLM 失败，已降级 mock：%s）" % reason)
			else:
				_append_system_message("（LLM 失败，已降级 mock）")
		"mock":
			if _api_mock_hint_shown:
				return
			_api_mock_hint_shown = true
			_append_system_message("（本地 mock · 非大模型；请检查服务是否 [LLM] 模式）")
...
func _begin_auto_seed_shop_flow(source_text: String) -> void:
...
func _try_handle_water_offer_reply(text: String) -> bool:
	...
	if GameState.get_unwatered_growing_plot_ids().is_empty():
		_pending_water_offer = false
		_append_companion_message("今天需要浇的田都浇过了。")
		return true
...
func _try_start_plant_from_correction(text: String) -> bool:
	if not _looks_like_empty_field_correction(text):
		return false
	return _start_companion_plant_task()
...
func _try_execute_companion_followthrough(reply_text: String, api_intent: Dictionary) -> void:
	...
	if ShopDelegate.looks_like_plant_commitment(reply_text):
	...
	if _looks_like_shop_seed_commitment(reply_text):
		if (
			GameState.get_item_count("turnip_seed") <= 0
			and not GameState.get_empty_plot_ids().is_empty()
			and TaskSystem.start_shop_task(true)
		):
			_begin_auto_seed_shop_flow(reply_text)
			_chat_action_handled = true
...
func _looks_like_empty_field_correction(text: String) -> bool:
	for phrase in ["有空田", "明明有空", "空着的", "空田", "两块空", "没种满", "还有空"]:
		if phrase in text:
			return true
	return false


func _looks_like_shop_seed_commitment(text: String) -> bool:
	return ("买" in text and "种子" in text) and ("商店" in text or "去买" in text)
...
func _append_companion_message(text: String) -> void:
	...
	_chat_log.append_text("\n[color=#ffd59a]%s：[/color] %s" % [GameState.companion_name, text])
...
func _apply_cozy_theme() -> void:
	...
	for panel in [$HUD, $TaskPanel, $ChatPanel]:
	...
	var input_style := panel_style.duplicate()
	input_style.bg_color = Color(1, 0.99, 0.96, 1)
	_chat_input.add_theme_stylebox_override("normal", input_style)
	_chat_input.add_theme_stylebox_override("focus", input_style.duplicate())
